...
from .config import (
    ...
    ADMIN_PASSWORD,
    ADMIN_USERNAME,
    GATE_SCAN_FAILURE_WINDOW_MINUTES,
    GLOBAL_CODE_SEND_LIMIT,
    GLOBAL_CODE_SEND_WINDOW_MINUTES,
    JWT_SECRET,
    LOGIN_2FA_EXPIRE_MINUTES,
    LOGIN_LOCK_MINUTES,
    MAX_FAILED_LOGIN_ATTEMPTS,
    MAX_GATE_SCAN_FAILURES,
    PASSWORD_MIN_LENGTH,
    REFRESH_TOKEN_EXPIRE_DAYS,
    REGISTRATION_CODE_EXPIRE_MINUTES,
    REGISTRATION_CODE_LENGTH,
    REGISTRATION_CODE_SEND_LIMIT,
    REGISTRATION_CODE_SEND_WINDOW_MINUTES,
    SMTP_FROM,
    SMTP_HOST,
    SMTP_PASSWORD,
    SMTP_PORT,
    ...
    TICKET_EMAIL_MIN_INTERVAL_SECONDS,
    TICKET_VALID_AFTER_HOURS,
    TICKET_VALID_BEFORE_HOURS,
...
from .schemas import (
    AuditEventOut,
    AuditFeedResponse,
    CashierRegisterRequest,
    CashierRegisterSendCodeRequest,
    CashierRegistrationRequestOut,
    EventCreate,
    EventOut,
    EventStatDetailsResponse,
    EventStatDetailItem,
    ...
    StaffRegisterSendCodeRequest,
    StatsByEventResponse,
    StatsOverviewResponse,
    STAT_DETAIL_CATEGORIES,
    TicketBatchSaleCreate,
    TicketBatchSaleResponse,
    TicketEmailRequest,
    TicketOut,
    TicketSaleCreate,
    TokenResponse,
    ...
    VenueOut,
    VenueUpdate,
    YandexGeocodeResultOut,
    YandexSuggestResponse,
    YandexSuggestItemOut,
)
...
REGISTRATION_CODE_SENDS_GLOBAL_UNIQUE: dict[str, list[datetime]] = {}
...
def can_send_registration_code(email_key: str) -> tuple[bool, int]:
    now = datetime.now(timezone.utc)
    per_email_window_start = now - timedelta(minutes=REGISTRATION_CODE_SEND_WINDOW_MINUTES)
    sends = [ts for ts in REGISTRATION_CODE_SENDS.get(email_key, []) if ts > per_email_window_start]
    REGISTRATION_CODE_SENDS[email_key] = sends
    if len(sends) >= REGISTRATION_CODE_SEND_LIMIT:
        retry_after = int((sends[0] - per_email_window_start).total_seconds())
        wait_seconds = max(1, REGISTRATION_CODE_SEND_WINDOW_MINUTES * 60 - retry_after)
        return False, wait_seconds

    global REGISTRATION_CODE_SENDS_GLOBAL_UNIQUE
    global_window_start = now - timedelta(minutes=GLOBAL_CODE_SEND_WINDOW_MINUTES)
    valid_unique: dict[str, list[datetime]] = {}
    for key, timestamps in REGISTRATION_CODE_SENDS_GLOBAL_UNIQUE.items():
        filtered = [ts for ts in timestamps if ts > global_window_start]
        if filtered:
            valid_unique[key] = filtered
    REGISTRATION_CODE_SENDS_GLOBAL_UNIQUE = valid_unique

    if email_key not in REGISTRATION_CODE_SENDS_GLOBAL_UNIQUE and len(REGISTRATION_CODE_SENDS_GLOBAL_UNIQUE) >= GLOBAL_CODE_SEND_LIMIT:
        oldest_first_send = min(ts_list[0] for ts_list in REGISTRATION_CODE_SENDS_GLOBAL_UNIQUE.values())
        retry_after = int((oldest_first_send - global_window_start).total_seconds())
        wait_seconds = max(1, GLOBAL_CODE_SEND_WINDOW_MINUTES * 60 - retry_after)
        return False, wait_seconds
    return True, 0
...
def register_registration_code_send(email_key: str) -> None:
    ...
    REGISTRATION_CODE_SENDS_GLOBAL_UNIQUE.setdefault(email_key, []).append(now)
